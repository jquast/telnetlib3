...
class TestMacroEditScreen:

    def test_macro_screen_loads_empty(self, tmp_path) -> None:
        path = str(tmp_path / "macros.json")
        screen = MacroEditScreen(path=path)
        assert screen._path == path
        assert screen._macros == []

    def test_macro_screen_loads_file(self, tmp_path) -> None:
        import json

        fp = tmp_path / "macros.json"
        fp.write_text(json.dumps({"macros": [{"key": "f5", "text": "look<CR>"}]}))
        screen = MacroEditScreen(path=str(fp))
        screen._load_from_file()
        assert len(screen._macros) == 1
        assert screen._macros[0] == ("f5", "look<CR>")

    def test_macro_screen_save(self, tmp_path) -> None:
        fp = tmp_path / "macros.json"
        screen = MacroEditScreen(path=str(fp))
...
class TestAutoreplyEditScreen:

    def test_autoreply_screen_loads_empty(self, tmp_path) -> None:
        path = str(tmp_path / "autoreplies.json")
        screen = AutoreplyEditScreen(path=path)
        assert screen._path == path
        assert screen._rules == []

    def test_autoreply_screen_loads_file(self, tmp_path) -> None:
        import json

        fp = tmp_path / "autoreplies.json"
        fp.write_text(
            json.dumps({"autoreplies": [{"pattern": r"\d+ gold", "reply": "get gold<CR>"}]})
        )
    ...
    def test_autoreply_screen_save(self, tmp_path) -> None:
        fp = tmp_path / "autoreplies.json"
        screen = AutoreplyEditScreen(path=str(fp))
    ...
    def test_autoreply_screen_rejects_bad_regex(self, tmp_path) -> None:
        import re
...
class TestHelpers:
    ...
    def test_relative_time_seconds_ago(self) -> None:
        past = datetime.datetime.now() - datetime.timedelta(seconds=30)
        result = _relative_time(past.isoformat())
        assert "30s ago" in result or "29s ago" in result

    def test_int_val_valid(self) -> None:
        assert _int_val("42", 0) == 42

    def test_int_val_fallback(self) -> None:
        assert _int_val("abc", 42) == 42

    def test_float_val_valid(self) -> None:
        assert _float_val("1.5", 0.0) == 1.5

    def test_float_val_fallback(self) -> None:
        assert _float_val("abc", 1.5) == 1.5

    def test_build_tooltips(self) -> None:
...
class TestTuiMain:
    ...
    def test_tui_main(self, monkeypatch) -> None:
        called = []
...
class _MacroEditApp(textual.app.App[None]):
    ...
    def on_mount(self) -> None:
        self.push_screen(MacroEditScreen(path=self._path), callback=lambda _: None)
...
class _AutoreplyEditApp(textual.app.App[None]):
    ...
    def on_mount(self) -> None:
        self.push_screen(AutoreplyEditScreen(path=self._path), callback=lambda _: None)
...
class TestAutoreplyEditScreenTextual:
    ...
    async def test_edit_autoreply(self, tmp_path) -> None:
        ...
        fp = tmp_path / "autoreplies.json"
        fp.write_text(json.dumps({"autoreplies": [{"pattern": "hello", "reply": "world<CR>"}]}))
        app = _AutoreplyEditApp(str(fp))
        async with app.run_test(size=(80, 24)) as pilot:
            await pilot.pause()
            screen = app.screen
...
class TestMacroEditScreenButtonDispatch:
    ...
    async def test_edit_submit_form(self, tmp_path) -> None:
        fp = tmp_path / "macros.json"
        fp.write_text('{"macros": [{"key": "f5", "text": "old<CR>"}]}')
        app = _MacroEditApp(str(fp))
        async with app.run_test(size=(80, 24)) as pilot:
            await pilot.pause()
            screen = app.screen
...
class TestAutoreplyEditScreenButtonDispatch:
    ...
    async def test_edit_submit_form(self, tmp_path) -> None:
        ...
        fp = tmp_path / "autoreplies.json"
        fp.write_text(json.dumps({"autoreplies": [{"pattern": "old", "reply": "old<CR>"}]}))
        app = _AutoreplyEditApp(str(fp))
        async with app.run_test(size=(80, 24)) as pilot:
            await pilot.pause()
            screen = app.screen
...
class TestSessionListCallbacks:
    ...
    async def test_on_macro_save_sets_path(self, tmp_path, monkeypatch) -> None:
        monkeypatch.setattr("telnetlib3.client_tui.SESSIONS_FILE", tmp_path / "s.json")
        monkeypatch.setattr("telnetlib3.client_tui.CONFIG_DIR", str(tmp_path))
        monkeypatch.setattr("telnetlib3.client_tui.DATA_DIR", str(tmp_path))
        sessions = {"srv1": SessionConfig(name="srv1", host="host1", macros_file="")}
        save_sessions(sessions)
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            await pilot.pause()
            screen = app.screen
            screen._on_macro_save("srv1", True, "/tmp/macros.json")
            await pilot.pause()
            assert screen._sessions["srv1"].macros_file == "/tmp/macros.json"

    async def test_on_autoreply_save_sets_path(self, tmp_path, monkeypatch) -> None:
        monkeypatch.setattr("telnetlib3.client_tui.SESSIONS_FILE", tmp_path / "s.json")
        monkeypatch.setattr("telnetlib3.client_tui.CONFIG_DIR", str(tmp_path))
        monkeypatch.setattr("telnetlib3.client_tui.DATA_DIR", str(tmp_path))
        sessions = {"srv1": SessionConfig(name="srv1", host="host1", autoreplies_file="")}
        save_sessions(sessions)
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            await pilot.pause()
            screen = app.screen
            screen._on_autoreply_save("srv1", True, "/tmp/autoreplies.json")
            await pilot.pause()
            assert screen._sessions["srv1"].autoreplies_file == "/tmp/autoreplies.json"
