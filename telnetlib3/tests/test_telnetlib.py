...
@pytest.fixture
def _mock_selector(monkeypatch):
...
def test_timeout_default(server_port):
...
def test_timeout_none(server_port):
    assert socket.getdefaulttimeout() is None
    socket.setdefaulttimeout(30)
    try:
...
def test_timeout_value(server_port):
    telnet = telnetlib.Telnet(HOST, server_port, timeout=30)
    assert telnet.sock.gettimeout() == 30
    telnet.sock.close()


def test_timeout_open(server_port):
...
def test_read_until(_mock_selector):
...
def test_read_all(_mock_selector):
...
def test_read_some(_mock_selector):
...
def test_read_eager(_mock_selector):
    _read_eager("read_eager")
    _read_eager("read_very_eager")


def read_very_lazy(_mock_selector):
    want = b"x" * 100
    telnet = make_telnet([want])
...
def test_read_lazy(_mock_selector):
    want = b"x" * 100
    telnet = make_telnet([want])
...
_OPTION_CMDS = [tl.AO, tl.AYT, tl.BRK, tl.EC, tl.EL, tl.GA, tl.IP, tl.NOP]


def _test_option_command(data):
...
def test_IAC_commands():
    for cmd in _OPTION_CMDS:
        _test_option_command([tl.IAC, cmd])
        _test_option_command([b"x" * 100, tl.IAC, cmd, b"y" * 100])
        _test_option_command([b"x" * 10, tl.IAC, cmd, b"y" * 10])
    _test_option_command([tl.IAC + cmd for cmd in _OPTION_CMDS])
...
def test_expect(_mock_selector):
    """Expect(expected, [timeout]) Read until the expected string has been seen, or a timeout is hit
    (default is no timeout); may block."""
