...
class TestRoomStore:
    ...
    def test_update_room_new(self, store: RoomStore) -> None:
        store.update_room({
            "num": "100",
            "name": "Town Square",
            "area": "midgaard",
    ...
    def test_update_room_existing(self, store: RoomStore) -> None:
        store.update_room({
            "num": "100", "name": "Town Square",
            "area": "midgaard", "exits": {"north": "101"},
        })
        store.update_room({
            "num": "100", "name": "Town Square (rebuilt)",
            "area": "midgaard", "exits": {"north": "101", "east": "103"},
        })
        r = store.get_room("100")
        assert r is not None
    ...
    def test_update_room_numeric_id(self, store: RoomStore) -> None:
        store.update_room({"num": 42, "name": "Numeric Room"})
        assert store.get_room("42") is not None

    def test_update_room_missing_optional_fields(self, store: RoomStore) -> None:
    ...
    def test_update_room_invalid_exits_ignored(self, store: RoomStore) -> None:
        store.update_room({"num": "1", "exits": "not-a-dict"})
        r = store.get_room("1")
        assert r is not None
        assert r.exits == {}

    def test_rooms_property(self, store: RoomStore) -> None:
    ...
    def test_get_room_missing(self, store: RoomStore) -> None:
        assert store.get_room("999") is None

    def test_close(self, tmp_path: Any) -> None:
...
class TestFindPath:
    ...
    def test_direct_neighbor(self, store: RoomStore) -> None:
        self._build_linear(store)
        assert store.find_path("A", "B") == ["east"]

    def test_multi_hop(self, store: RoomStore) -> None:
        self._build_linear(store)
        assert store.find_path("A", "C") == ["east", "east"]

    def test_reverse_path(self, store: RoomStore) -> None:
        self._build_linear(store)
        assert store.find_path("C", "A") == ["west", "west"]

    def test_same_room(self, store: RoomStore) -> None:
        self._build_linear(store)
        assert store.find_path("A", "A") == []

    def test_no_path(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
        store.update_room({"num": "B", "exits": {}})
        store.update_room({"num": "C", "exits": {"west": "B"}})
        assert store.find_path("A", "C") is None

    def test_unknown_src(self, store: RoomStore) -> None:
        assert store.find_path("X", "Y") is None

    def test_one_way_exits(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"down": "B"}})
        store.update_room({"num": "B", "exits": {}})
        assert store.find_path("A", "B") == ["down"]
        assert store.find_path("B", "A") is None

    def test_cycle_handling(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
        store.update_room({"num": "B", "exits": {"east": "C"}})
        store.update_room({"num": "C", "exits": {"east": "A"}})
        assert store.find_path("A", "C") == ["east", "east"]

    def test_target_not_in_graph_but_reachable(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
        assert store.find_path("A", "B") == ["east"]

    def test_find_path_with_rooms(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
    ...
    def test_find_path_with_rooms_same(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {}})
        assert store.find_path_with_rooms("A", "A") == []

    def test_bfs_distances(self, store: RoomStore) -> None:
        self._build_linear(store)
        d = store.bfs_distances("A")
        assert d == {"A": 0, "B": 1, "C": 2}

    def test_bfs_distances_unreachable(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {}})
        store.update_room({"num": "B", "exits": {}})
        d = store.bfs_distances("A")
        assert d == {"A": 0}

    def test_bfs_distances_unknown_src(self, store: RoomStore) -> None:
        assert not store.bfs_distances("X")


class TestFindSameName:

    def test_basic_same_name(self, store: RoomStore) -> None:
        store._conn.execute(
    ...
    def test_excludes_self(self, store: RoomStore) -> None:
        store._conn.execute(
            "INSERT INTO room (num, name, last_visited) VALUES (?, ?, ?)",
            ("1", "Forest", "2024-01-01"),
        )
    ...
    def test_missing_room(self, store: RoomStore) -> None:
        assert store.find_same_name("999") == []

    def test_empty_name(self, store: RoomStore) -> None:
        store._conn.execute(
            "INSERT INTO room (num, name) VALUES (?, ?)", ("1", "")
        )
        store._conn.execute(
            "INSERT INTO room (num, name) VALUES (?, ?)", ("2", "")
        )
        store._conn.commit()
        assert store.find_same_name("1") == []

    def test_no_matches(self, store: RoomStore) -> None:
        store.update_room({"num": "1", "name": "Unique Room"})
        store.update_room({"num": "2", "name": "Different Room"})
        assert store.find_same_name("1") == []

    def test_never_visited_sort_first(self, store: RoomStore) -> None:
        store._conn.execute(
            "INSERT INTO room (num, name, last_visited) VALUES (?, ?, ?)",
            ("1", "Road", "2024-01-01"),
        )
    ...
    def test_limit(self, store: RoomStore) -> None:
        store._conn.execute(
            "INSERT INTO room (num, name, last_visited) VALUES (?, ?, ?)",
            ("0", "Road", "2024-01-01"),
        )
    ...
    def test_default_limit_99(self, store: RoomStore) -> None:
        store._conn.execute(
            "INSERT INTO room (num, name, last_visited) VALUES (?, ?, ?)",
            ("0", "Road", "2024-01-01"),
        )
...
class TestBookmarkAndSearch:
    ...
    def test_toggle_bookmark(self, store: RoomStore) -> None:
    ...
    def test_toggle_bookmark_missing_room(self, store: RoomStore) -> None:
        assert store.toggle_bookmark("999") is False

    def test_search_by_name(self, store: RoomStore) -> None:
    ...
    def test_search_by_area(self, store: RoomStore) -> None:
    ...
    def test_search_case_insensitive(self, store: RoomStore) -> None:
        store.update_room({"num": "1", "name": "DARK FOREST"})
        assert len(store.search("dark")) == 1
        assert len(store.search("DARK")) == 1

    def test_search_bookmarked_first(self, store: RoomStore) -> None:
    ...
    def test_search_empty_query(self, store: RoomStore) -> None:
        store.update_room({"num": "1", "name": "Room A"})
        store.update_room({"num": "2", "name": "Room B"})
        assert len(store.search("")) == 2


class TestRoomGraphAlias:

    def test_alias_is_room_store(self) -> None:
...
class TestPathHelpers:
    ...
    def test_session_file_path_traversal(self, malicious_key: str) -> None:
...
class TestFindBranches:
    ...
    def test_finds_frontier_exit(self, store: RoomStore) -> None:
        self._build_graph(store)
        branches = store.find_branches("A")
        dirs = [(gw, d) for gw, d, _ in branches]
        assert ("A", "north") in dirs

    def test_unknown_target_is_frontier(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
        branches = store.find_branches("A")
        assert len(branches) == 1
        assert branches[0] == ("A", "east", "B")

    def test_unvisited_target_is_frontier(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
        store._conn.execute(
            "INSERT INTO room (num, name, visit_count) VALUES (?, ?, ?)",
            ("B", "Empty", 0),
        )
        store._conn.commit()
        branches = store.find_branches("A")
    ...
    def test_visited_target_not_frontier(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
        store.update_room({"num": "B", "exits": {"west": "A"}})
        branches = store.find_branches("A")
        assert len(branches) == 0

    def test_sorted_by_distance(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
    ...
    def test_empty_graph(self, store: RoomStore) -> None:
    ...
    def test_limit(self, store: RoomStore) -> None:
        store.update_room(
            {"num": "A", "exits": {"n": "X1", "s": "X2", "e": "X3"}}
        )
        branches = store.find_branches("A", limit=2)
        assert len(branches) == 2

    def test_no_duplicates(self, store: RoomStore) -> None:
        store.update_room({"num": "A", "exits": {"east": "B"}})
        store.update_room({"num": "B", "exits": {"east": "C", "west": "A"}})
        branches = store.find_branches("A")
